...
- **Kapanış:** 2026-09-24 — **TB-014: Harf harf okunan kısaltmalar yanlış ek alıyor.**
  - **Ne yapıldı (sade):** Hiç ünlü içermeyen büyük harf kısaltmalar (THY, PTT, SGK, BMW) artık son harfin okunuşuna göre ek alıyor: "THY'de", "PTT'de". Çalışırken daha ciddi bir hata bulundu ve o da düzeltildi: "-aş" ile biten her ad (Kocataş, Karataş, BOTAŞ) şirket kısaltması "AŞ" sanılıyordu: "Ahmet Kocataş'ne".
  - **Ölçülen sonuç:** Otorite TDK Yazım Kuralları "Kısaltmalar" sayfası; oradaki 11 örneğin 11'i doğru (`BDT'ye`, `TDK'den`, `THY'de`, `TRT'den`, `TL'nin`, `ASELSAN'da`, `BOTAŞ'ın`, `NATO'dan`, `UNESCO'ya`, `AGİK'in`, `RTÜK'e`). Düzeltmeden önce `BOTAŞ'ın` da yanlıştı (`BOTAŞ'nin`). 11 mutasyonun 11'i kırmızı. Kayıt açılırken yazılan `SGK'da` beklentisi TDK'ye uymuyordu (TDK: K "ke", `TDK'den`). Sahip, halk okunuşunu ("ka") seçti: `SGK'da`, `TDK'dan` — sahip kararıyla istisna (`GERI-BILDIRIM-KAYDI.md` madde 44D). TDK'nın 11 örneğinden `TDK'den` bu yüzden bilerek farklı çıkıyor.
  - **Bilinen sınır (kapatılmadı, belgelendi):** Ünlü içeren ama harf harf okunan kısaltmalar (`ABD`, doğrusu `ABD'ye`) kelime gibi okunan kısaltmalarla (`AGİK`, `RTÜK`) aynı biçimde yazılıyor ve metinden ayırt edilemiyor. Çekirdek tahmin etmez; `GERI-BILDIRIM-KAYDI.md` madde 44C yeniden başvuru koşulunu taşıyor.
  - **Ders:** Beklenen değer bile kaynaktan okunmalı. Kaydı açarken `SGK'da`yı gündelik okunuştan yazdım, TDK tersini gösterdi; iki kaynak çelişince karar sahibe götürüldü ve sahip halk okunuşunu seçti. Ayrıca bir sorunun yanındaki kod okunmalı: "-aş" hatası TB-014'ün aradığı şey değildi ama aynı işlevin aynı parçasıydı ve müşteriye giden mesajı bozuyordu.
